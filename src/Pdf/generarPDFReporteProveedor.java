...
class generarPDFReporteProveedor {
...
generarPDFReporteProveedor(String titulo,int proveedor,String fechaDel, String fechaAl,String nombreProveedor) {
 Colocar_Destino();
 strRotuloPDF = titulo;
 if(this.ruta_destino!=null){
  try {
   ...
   agregarMetaDatos(document,proveedor,fechaDel,fechaAl,nombreProveedor);
   document.close();
   this.abrir();
  }
  catch (Exception e){
...
private void agregarMetaDatos(Document document,int dni,String fechaDel,String fechaAl,String nombreProveedor) throws DocumentException {
 Paragraph ParrafoHoja = new Paragraph();
  agregarLineasEnBlanco(ParrafoHoja, 1);
  ParrafoHoja.add(new Paragraph(strRotuloPDF.toUpperCase ()));
  agregarLineasEnBlanco(ParrafoHoja, 1);
  agregarTabla(ParrafoHoja,dni,fechaDel,fechaAl,nombreProveedor);
...
private void agregarTabla(Paragraph parrafo,int proveedor,String fechaDel, String fechaAl,String nombreProveedor) {
 float anchosFilas[] = { 1.2f,0.5f,0.6f,0.6f,0.7f,1f};
     PdfPTable tabla = new PdfPTable(anchosFilas);
     String rotulosColumnas[] = {"Producto","Precio","Cantidad","SubTotal","Total","fecha"};
     tabla.setWidthPercentage(100);
     tabla.setHorizontalAlignment(Element.ALIGN_CENTER);
     PdfPCell cell = new PdfPCell(new Paragraph("Compras del Proveedor: " + nombreProveedor));
     cell.setColspan(6);
	...
	cell.setHorizontalAlignment(Element.ALIGN_CENTER);
      ...
      while (rs.next()){
       ...
       tabla.addCell(cell);
       cell = new PdfPCell(new Paragraph(String.valueOf(rs.getDate("com.Fecha"))));
       tabla.addCell(cell);
      }
...
void abrir() {
 JOptionPane.showMessageDialog(null,"Se ha generado el PDF", "Confirmacion", JOptionPane.ERROR_MESSAGE);
}
