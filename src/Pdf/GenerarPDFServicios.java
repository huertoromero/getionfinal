...
public class GenerarPDFServicios {
...
public GenerarPDFServicios(String titulo) {
 Colocar_Destino();
 strRotuloPDF = titulo;
  try {
...
private void agregarTabla(Paragraph parrafo) {
 ...
 PdfPCell cell = new PdfPCell(new Paragraph("SERVICOS"));
 cell.setColspan(9);
 ...
 try {
  ...
  strConsultaSQL = "SELECT ser.idServicio, ser.nombre, ser.precio FROM servicios ser";
  rs = estSQL1.executeQuery(strConsultaSQL);
  while (rs.next()){
  cell = new PdfPCell(new Paragraph(String.valueOf(rs.getInt("ser.idServicio"))));
  tabla.addCell(cell);
  cell = new PdfPCell(new Paragraph(String.valueOf(rs.getString("ser.nombre"))));
  tabla.addCell(cell);
  cell = new PdfPCell(new Paragraph(String.valueOf(rs.getFloat("ser.precio"))));
  tabla.addCell(cell);
  }
...
private void Colocar_Destino() {
        this.ruta_destino_1="C:\\Reportes\\Servicios\\ListadoDeServicios "+ventanaPrincipal.listServicios++;
