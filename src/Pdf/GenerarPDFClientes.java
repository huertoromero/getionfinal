...
public class GenerarPDFClientes {
    ...
    public GenerarPDFClientes(String titulo) {
        Colocar_Destino();
        strRotuloPDF = titulo;
        if (true) {
    ...
    private void agregarTabla(Paragraph parrafo) {
        float anchosFilas[] = {1f, 2f, 2f, 1.5f, 1.5f};
        PdfPTable tabla = new PdfPTable(anchosFilas);
        String rotulosColumnas[] = {"Dni", "Nombre", "Mail", "Telefono", "Direccion"};
        tabla.setWidthPercentage(150);
        tabla.setHorizontalAlignment(Element.ALIGN_CENTER);
        PdfPCell cell = new PdfPCell(new Paragraph("CLIENTES"));
        cell.setColspan(12);
        ...
        try {
            if (ConectarBD()) {
                ...
                strConsultaSQL = "SELECT * FROM clientes WHERE estado = 1";
...
//  if ( result == JFileChooser.APPROVE_OPTION ){
        ...
        this.ruta_destino_1 = "C:\\Reportes\\Clientes\\ListadoDeCliente " + ventanaPrincipal.listCliente++;
