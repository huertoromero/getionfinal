...
import com.itextpdf.text.Paragraph;import java.awt.Color;
;
...
public PDFventasClienteyFecha(String titulo,int dni,String fechaDel,String fechaAl) {
 Colocar_Destino();
 strRotuloPDF = titulo;
 if(this.ruta_destino!=null){
  try {
   ...
   document.add(jpg);
   agregarMetaDatos(document,dni,fechaDel,fechaAl);
   document.close();
   this.abrir();
  }
  catch (Exception e){
...
private void agregarMetaDatos(Document document,int dni,String fechaDel,String fechaAl) throws DocumentException {
 Paragraph ParrafoHoja = new Paragraph();
  agregarLineasEnBlanco(ParrafoHoja, 1);
  ParrafoHoja.add(new Paragraph(strRotuloPDF.toUpperCase ()));
  agregarLineasEnBlanco(ParrafoHoja, 1);
  agregarTabla(ParrafoHoja,dni,fechaDel,fechaAl);
...
private void agregarTabla(Paragraph parrafo,int dni,String fechaDel,String fechaAl) {
 float anchosFilas[] = { 0.7f,0.7f,0.7f,0.7f,1f};
     PdfPTable tabla = new PdfPTable(anchosFilas);
     String rotulosColumnas[] = {"Numero de Venta","Dni Cliente","Fecha","Hora","Total"};
     tabla.setWidthPercentage(100);
     tabla.setHorizontalAlignment(Element.ALIGN_CENTER);
     PdfPCell cell = new PdfPCell(new Paragraph("Ventas del Cliente: " + dni));
     cell.setColspan(8);
	...
	cell.setHorizontalAlignment(Element.ALIGN_CENTER);
      ...
      strConsultaSQL = "SELECT idVentas,dniCliente,Fecha,Hora,Total FROM ventas vent WHERE dniCliente = '" + dni +"' AND Fecha BETWEEN '" + fechaDel + "' AND '" + fechaAl + "'";
      rs = estSQL1.executeQuery(strConsultaSQL);
      while (rs.next()){
       cell = new PdfPCell(new Paragraph(String.valueOf(rs.getInt ("idVentas"))));
       tabla.addCell(cell);
       cell = new PdfPCell(new Paragraph(String.valueOf(dni)));
       tabla.addCell(cell);
       cell = new PdfPCell(new Paragraph(String.valueOf(rs.getString("Fecha"))));
       tabla.addCell(cell);
       cell = new PdfPCell(new Paragraph(String.valueOf(rs.getString("Hora"))));
       tabla.addCell(cell);
