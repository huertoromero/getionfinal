...
public boletaVenta(int idVenta,int dni) throws Exception{
Colocar_Destino(idVenta);
FechayHora fec = new FechayHora();
  try {
   ...
   document.addAuthor("ABC software");
   ...
   ParrafoHoja1.add(new Paragraph("Schick",FontFactory.getFont("Times New Roman",48, Font.NORMAL, BaseColor.BLACK)));
   ParrafoHoja1.setAlignment(2);
   document.add(ParrafoHoja1);
   agregarLineasEnBlanco(ParrafoHoja1,1);
   ParrafoHoja2.add(new Paragraph("Av Mitre 445 - TUCUMÁN",FontFactory.getFont("arial",10, Font.NORMAL, BaseColor.BLACK)));
   ...
   document.close();
   //abrir(this.ruta_destino.getAbsolutePath());
...
private void agregarTabla(Paragraph parrafo,int idVenta) {
 float anchosFilas[] = { 1.2f,0.5f};
 PdfPTable tabla = new PdfPTable(anchosFilas);
 String rotulosColumnas[] = {"Servicio","SubTotal"};
...
//    strConsultaSQL= "SELECT ser.nombre, dv.subTotal FROM servicios ser INNER JOIN detalle_venta dv ON dv.servicio_id=ser.idServicio AND dv.venta_id='" + idVenta +"'";
    strConsultaSQL= "SELECT ser.nombre, dv.subTotal FROM servicios ser, detalle_venta dv WHERE dv.servicio_id=ser.idServicio AND dv.venta_id='" + idVenta +"'";

  rs = estSQL1.executeQuery(strConsultaSQL);
  while (rs.next()){
  
  cell = new PdfPCell(new Paragraph(rs.getString("ser.nombre")));
//  cell.setBorder(0);
  tabla.addCell(cell);
  cell = new PdfPCell(new Paragraph(String.valueOf(rs.getFloat("dv.subTotal"))));
//  cell.setBorder(0);
  tabla.addCell(cell);
//  cell.setBorder(1);
 }
...
private void Colocar_Destino(int nroVenta) {
/*FileNameExtensionFilter filter = new FileNameExtensionFilter("Archivo PDF","pdf","PDF");
 ...
 int result = fileChooser.showSaveDialog(null);
  if ( result == JFileChooser.APPROVE_OPTION ){
    ...
    //voy a mostrar variebles en el metodo Colocar_destino
    System.out.println("FILECHOOSER: "+fileChooser.getSelectedFile());
    
    System.out.println("");
    System.out.println("");
  }*/
    this.ruta_destino_1="C:\\Reportes\\Ventas\\Factura "+nroVenta;
 }

private void abrir(String ruta) {
    System.out.println("ENTROOO A ABRIR Y SE TIENE Q ABRIR");
 try {
     JOptionPane.showMessageDialog(null,"Se ha generado la factura", "Confirmacion", JOptionPane.INFORMATION_MESSAGE);
...
private String total(int idVenta) throws Exception {
  float total=0;
  ConectarBD();
  strConsultaSQL= "SELECT total FROM ventas WHERE idVenta ='" + idVenta + "'";
  rs = estSQL1.executeQuery(strConsultaSQL);
  if(rs.next()==true){
 ...
 private String monto(int idVenta) throws Exception {
  float monto =0;
  ConectarBD();
  strConsultaSQL= "SELECT Monto FROM ventas WHERE idVenta ='" + idVenta + "'";
  rs = estSQL1.executeQuery(strConsultaSQL);
  if(rs.next()==true){
 ...
 private String vuelto(int idVenta) throws Exception {
  float vuelto =0;
  ConectarBD();
  strConsultaSQL= "SELECT Vuelto FROM ventas WHERE idVenta ='" + idVenta + "'";
  rs = estSQL1.executeQuery(strConsultaSQL);
  if(rs.next()==true){
 ...
 private String fecha(int idVenta) throws Exception {
  String fecha = "";
  ConectarBD();
  strConsultaSQL= "SELECT Fecha FROM ventas WHERE idVenta ='" + idVenta + "'";
  rs = estSQL1.executeQuery(strConsultaSQL);
  if(rs.next()==true){
...
private String hora(int idVenta) throws Exception {
 String hora = "";
  ConectarBD();
  strConsultaSQL= "SELECT Hora FROM ventas WHERE idVenta ='" + idVenta + "'";
  rs = estSQL1.executeQuery(strConsultaSQL);
  if(rs.next()==true){
...
private String buscarVendedor(int idVenta) throws Exception {
 String hora = "";
  ConectarBD();
  strConsultaSQL= "SELECT dniEmpleado FROM ventas WHERE idVenta ='" + idVenta + "'";
  rs = estSQL1.executeQuery(strConsultaSQL);
  if(rs.next()==true){
