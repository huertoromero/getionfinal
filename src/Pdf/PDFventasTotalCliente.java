...
public class PDFventasTotalCliente {
...
public PDFventasTotalCliente (String titulo,int dni) {
 Colocar_Destino();
 strRotuloPDF = titulo;
...
private void agregarMetaDatos(Document document,int dni) throws DocumentException {
 Paragraph ParrafoHoja = new Paragraph();
  agregarLineasEnBlanco(ParrafoHoja, 1);
  ParrafoHoja.add(new Paragraph(strRotuloPDF.toUpperCase ()));
  agregarLineasEnBlanco(ParrafoHoja, 1);
  agregarTabla(ParrafoHoja,dni);
...
private void agregarTabla(Paragraph parrafo,int dni) {
 float anchosFilas[] = { 0.7f,0.7f,0.7f,0.7f,1f};
	...
	cell.setHorizontalAlignment(Element.ALIGN_CENTER);
      ...
      strConsultaSQL = "SELECT idVentas,dniCliente,Fecha,Hora,Total FROM ventas vent WHERE dniCliente = '" + dni +"'";
