...
public class GenerarPDFProveedores {
...
public GenerarPDFProveedores(String titulo) {
 Colocar_Destino();
 strRotuloPDF = titulo;
  try {
...
private void agregarTabla(Paragraph parrafo) {
 float anchosFilas[] = { 1.5f,1f,2f,1.2f,1.5f};
 PdfPTable tabla = new PdfPTable(anchosFilas);
 String rotulosColumnas[] = {"Razon Social","Telefono","E-Mail","Provincia","Direcion"};
 tabla.setWidthPercentage(150);
 tabla.setHorizontalAlignment(Element.ALIGN_CENTER);
 PdfPCell cell = new PdfPCell(new Paragraph("PROVEEDORES"));
 cell.setColspan(12);
 ...
 try {
  ...
  strConsultaSQL = "SELECT * FROM proveedor WHERE Estado = 1";
  rs = estSQL1.executeQuery(strConsultaSQL);
  while (rs.next()){
  cell = new PdfPCell(new Paragraph(rs.getString ("RazonSocial")));
  tabla.addCell(cell);
  cell = new PdfPCell(new Paragraph(String.valueOf(rs.getLong("Telefono"))));
  tabla.addCell(cell);
  cell = new PdfPCell(new Paragraph(String.valueOf(rs.getString("EMail"))));
  tabla.addCell(cell);
  cell = new PdfPCell(new Paragraph(String.valueOf(rs.getString("provincia"))));
  tabla.addCell(cell);
...
private void Colocar_Destino() {
        this.ruta_destino_1="C:\\Reportes\\Proveedores\\ListadoDeProveedores "+ventanaPrincipal.listProveedores++;
